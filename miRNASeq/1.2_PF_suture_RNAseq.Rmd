---
title: "PF suture RNAseq in gene mode"
author: "Huan Liu"
date: "2024-02-18"
...
knitr::opts_chunk$set(
  ...
  cache.lazy = FALSE)
```
Here we analyzed DEGs in PF suture RNA-seq results in gene mode.
...
design_matrix <- data.frame(
  sample = c("D0_PF_1", "D0_PF_2", "D7_PF_1", "D7_PF_2"),
...
# 4 Sleuth process
Again, here we use gene mode to prepare sleuth. However, if you care about transcript, turn off the gene_mode, and check the transcript first.
...
```{r,gene expression plot for Egr1}
plot_bootstrap(so1, "Egr1", units = "est_counts", color_by = "condition")
...
write.csv(sleuth_significant, file="./PF_suture_RNAseq_sig.csv",row.names = FALSE)
```
...
write.csv(sleuth_geneexp, file="./PF_suture_RNAseq_exp.csv",row.names = FALSE)
```
...
write.csv(sig_exp, file = "./PF_suture_RNAseq_sig_exp.csv", row.names = FALSE)
```
## X R session info
```{r,R session info}
sessionInfo()
